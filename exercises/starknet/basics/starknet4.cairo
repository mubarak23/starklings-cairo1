...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_quantity: u32 ) {
        ...
        fn purchase(ref self: ContractState, product: u32) {
            let curr_stock = self.inventory.read(product);
            self.inventory.write(product, curr_stock - quantity);
            // TODO:
